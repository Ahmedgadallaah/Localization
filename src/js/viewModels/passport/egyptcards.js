...
define(['ojs/ojcore', 'knockout', 'jquery',
  ...
  function (oj, ko, $, ipConfig, fetchAllDataClass) {
    function EgyptCards() {
      ...
      // self.url = 'http://' + ipConfig.firstOctet + '.' + ipConfig.secondOctet + '.' + ipConfig.thirdOctet + '.' + ipConfig.fourthOctet + ':' + ipConfig.portNumber + '/ExitEntryEgy_SBProject/ExitEntryEgyRestService';
      ...
      //console.log("POST DATA GET DATA", self.postData);
      ...
      self.doSearch = function () {
        ...
        self.fetchAllData.getData(self.errorMessages).
        then(function (values) {
          var info = values[0];
          // var retrivedData = JSON.parse(JSON.stringify(data.exitEntryEgyOutput));
          if (info.isOK)
            self.redrivedData(info.data.exitEntryEgyOutput);
        });
      }

      self.connected = function () {

        //console.log("URL Service Egypt Cards", self.url);
